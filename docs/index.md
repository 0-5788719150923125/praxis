...
Praxis is built around 54 pluggable registries. The feature categories below link to a page listing the registered implementations and their source.

## Feature registries

- [Activation combination types](activation-types.md) (5) - How the `values` of an ``--activation-type`` combine at the gate. Declared as ``{type: <one of these>, values: [<activations>]}``.
