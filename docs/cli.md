...
| `--diffusion-steps` | int | `16` | Refinement steps per sample at generation time. Fewer steps commits more positions at once, which assumes they are conditionally independent; quality-vs-steps is the first thing worth measuring |
| `--diffusion-type` | str | `None` | Train as a non-autoregressive diffusion model: the sequence is corrupted and reconstructed in place, attention is bidirectional, labels are unshifted, and generation is iterative refinement. Unset = ordinary next-token training (choices: masked, masked_coarse) |
...
| `--encoder-type` | str | `None` | Encoder integration to use (choices: byte_latent, byte_latent_conv, byte_latent_conv_small, byte_latent_transformer, byte_latent_transformer_windowed, abstractinator_rvq, abstractinator_v0, abstractinator_v1, abstractinator_v1_calm, abstractinator_v2, abstractinator_v3, abstractinator_v2_additive, calm, calm_small, calm_byte, calm_byte_small, calm_byte_small_harmonic, calm_byte_ref, calm_byte_flow, calm_byte_harmonic, calm_byte_fixed, calm_byte_hybrid, calm_byte_harmonic_codec, calm_byte_harmonic_serpent, calm_tm_ref, calm_bpe) |
...
| `--ffn-type` | str | `glu` | The feedforward-network implementation to use within each block (choices: mlp, glu, arc, poly, scatter, kan, peer, peer_glu, peer_glu_single_head, eml_tree, spline) |
...
| `--sorting-type` | str | `none` | The type of feature sorting to use (choices: none, decay_bias, amplitude_field, native, sinkhorn) |
...
| `--peft-type` | str | `None` | Adapter profile to finetune --model-name with (choices: lora, lora_wide, lora_attention, rslora, dora) |
...
| `--gradient-clip-val` | float | `10.0` | Global gradient-norm clip threshold. Watch the optimizer card's Clip Rate to tell whether it's binding. Ignored by trainers that don't support clipping (mono_forward/pipeline). |
| `--loss-func` | str | `cross_entropy` | The loss function to use (choices: cross_entropy, dedup, focal, focal_alpha, mode_cross_entropy, mode_baseline_cross_entropy, hem, hem_plus, mile, stablemax, contrastive_token, halo, cut_cross_entropy) |
...
| `--rl-type` | str | `None` | Enable reinforcement learning with specified algorithm. Note: Current GRPO implementation uses static dataset rewards (not true RL). True RL with generation will be added in a future update. (choices: cot, engagement, grpo, harmonic_weight, harmonic_weight_anchor, harmonic_weight_wave, joke, preference, reinforce) |
| `--sampler-mode` | str | `loss` | Dataset sampling weighting mode: 'loss' (upsample high-loss datasets via per-sequence CE), 'tasker' (sample each dataset by its task's learned loss weight from --task-weights; pairs with a 'difficulty' weighter to upsample hard tasks), 'novelty' (bigram novelty via Count-Min Sketch), 'dynamic' (EMA of token counts), 'static' (use weights from DATASET_COLLECTIONS as-is), or 'uniform' (force every dataset to 1.0, ignoring DATASET_COLLECTIONS). (choices: novelty, dynamic, static, loss, tasker, uniform) |
| `--seq-curriculum` | str | `fixed` | Sequence-length curriculum. 'fixed' rolls the static per-tier chances. 'probe' fits the mix by regressing a held-out probe's improvement onto the arm mixture that produced it, so each length's value is measured in held-out loss. See praxis/data/seq_probe.py. (choices: fixed, probe) |
| `--train-datasets` | str | `['base']` | Named dataset collections to train on. Space- or comma-separated (e.g. '--train-datasets base phi' or '--train-datasets base,phi'). Available: base, chat, cot, dev, focused, git_history, joke, phi, pile, praxis, print, rl, slimpajama, tools, validation. |
...
| `--generation-mode` | str | `None` | How to decode at inference. 'standard' is the model's own loop (byte-level, MTP speculative decode where it applies); 'vote' is CALM's count-based patch vote. Inference-only and excluded from the run hash, so both can be compared on one checkpoint. Omit to let the encoder choose. (choices: standard, vote) |
