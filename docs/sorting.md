...
Optional reordering operations applied to the sequence. The slot also hosts differentiable positional-bias fields (``decay_bias``, ``amplitude_field``) that reuse the hook without permuting anything; a module's ``permutes`` attribute says which kind it is.

Namespace: ``registry.namespace("sorting")``, declared in ``praxis.sorting.base`` (5 entries)

Selected with ``--sorting-type`` (default: ``none``).
