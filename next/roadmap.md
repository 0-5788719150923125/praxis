...
- [x] **Environment variable support for CLI arguments**
  Every argparse flag (including dynamically added ones from integrations and experiment/environment discovery) is now settable via `PRAXIS_<DEST>` env vars. `EnvVarLoader` (`praxis/cli/loaders/env_vars.py`) iterates `parser._actions` after parsing, coerces the raw string to the action's declared type (bool-ish for store_true, type callable for scalars, comma or JSON list for `nargs`/`append`/`extend`, validated against `choices`), and applies it unless the CLI flag was given explicitly. Precedence top-down: explicit CLI > active environment YAML > `PRAXIS_*` env var > experiment YAML > argparse default.
...
- [ ] **Modularize dataset management with named collections**
  Today every curated dataset is lumped into the single `--phi` boolean flag, which forces an all-or-nothing tradeoff and hides which sources are actually in the mix. Replace this with named dataset collections (e.g. `--datasets phi`, `--datasets code`, `--datasets reasoning`) that the user can pick from, compose with list arguments (e.g. `--datasets phi,code`), or define ad-hoc via a list of dataset identifiers. Collections should be declared in one place so they are discoverable, and individual datasets should remain callable on their own without requiring a collection wrapper.
