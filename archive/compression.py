...
class TokenMerger(nn.Module):
    ...
    def __init__(self, interpolation_method="linear"):
        ...
        if interpolation_method not in self.supported_modes:
            raise ValueError(
    ...
    def reduce_sequence_by_factor(self, sequence, factor):
        """
        ...
        target_length = max(1, int(seq_length * factor))
    ...
    def reduce_sequence(self, sequence, target_length):
        """
        Reduce sequence length through interpolation.
    ...
    def reduce_block_ids_by_factor(self, block_ids, factor):
        """
    ...
    # Legacy method for backward compatibility
    def reduce_block_ids(self, block_ids, target_length):
        """
        Reduce block IDs to match a reduced token sequence length while preserving
        block structure and boundaries.
...
if __name__ == "__main__":
    ...
    print("Testing TokenMerger with interpolation-based sequence reduction...")
    ...
    for method in ["linear", "nearest"]:
        ...
        # Initialize the TokenMerger
        merger = TokenMerger(interpolation_method=method)
        ...
        for factor in [0.9, 0.7, 0.5]:
            ...
            reduced = merger.reduce_sequence_by_factor(sequence, factor)
    ...
    merger = TokenMerger(interpolation_method="linear")
    ...
    for factor in [0.75, 0.5]:
        ...
        reduced_block_ids = merger.reduce_block_ids_by_factor(block_ids, factor)
    ...
    reduced = merger.reduce_sequence_by_factor(large_sequence, factor)
    ...
    start_time = time.time()
    reduced_block_ids = merger.reduce_block_ids_by_factor(large_block_ids, factor)
    ...
    factor = 0.75
    reduced_embeddings = merger.reduce_sequence_by_factor(embeddings, factor)
    reduced_block_ids = merger.reduce_block_ids_by_factor(block_ids, factor)
