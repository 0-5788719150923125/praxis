"""Weights & Biases integration class for Praxis."""
...
class Integration(BaseIntegration):
    ...
    def initialize(
        ...
        if ckpt_path is not None:
            run_id = self._find_latest_wandb_run(cache_dir, cleanup_old_runs=True)
    ...
    def provide_logger(
        ...
        if not args or not getattr(args, "wandb", False):
            return None

        # Initialize wandb if not already done
        if not self._initialized:
            init_result = self.initialize(args, cache_dir, ckpt_path, truncated_hash)
        else:
            # If already initialized, get the config
            init_result = self.initialize(args, cache_dir, ckpt_path, truncated_hash)
    ...
    def _find_latest_wandb_run(self, cache_dir: str, cleanup_old_runs: bool = True) -> Optional[str]:
        """
