...
from virtual_camera import PreviewWindow, VirtualCamera
...
def parse_args():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Show debug information on video"
    )
    parser.add_argument(
        "--color",
        type=str,
...
def main():
    ...
    try:
        ...
        print(
            f"Error: Invalid color format '{args.color}'. Use BGR format like '0,255,136'"
        )
        return 1
    ...
    capture = VideoCapture(
        src=args.camera, width=args.width, height=args.height, fps=args.fps
    )
    ...
    while True:
        ...
        if face_data is not None:
            ...
            with monitor.measure("pose"):
                ...
                rotation_matrix, (yaw, pitch, roll) = pose_calc.calculate_pose(
                    points_2d, points_3d
                )
        ...
        with monitor.measure("rendering"):
            output_frame = renderer.render(
                frame_highres, jaw_points, jaw_width, pose_data
            )
