...
def apply_temporal_smoothing(
    predictions: list, method: str, window_size: int, consensus_threshold: float = 0.6
) -> list:
    """
    ...
    if method == "median":
        ...
        smoothed_probs = np.convolve(
            probs, np.ones(window_size) / window_size, mode="same"
        )
...
def apply_threshold_to_predictions(
    predictions: list, threshold: float, task_config: dict = None
) -> list:
    """
...
def detect_events(
    predictions: list, min_duration: float, task_config: dict = None
) -> list:
    """
...
def process_events_from_predictions(
    ...
    if config and config.get("inference", {}).get("temporal_smoothing", {}).get(
        "enabled", False
    ):
    ...
    predictions = apply_threshold_to_predictions(
        predictions_to_threshold, threshold, task_config
    )

    positive_frames = sum(1 for p in predictions if p["predicted_class"] == pos_label)
    print(
        f"{pos_label} frames: {positive_frames}/{len(predictions)} ({positive_frames/len(predictions)*100:.1f}%)"
    )
    ...
    if video_path and os.path.exists(video_path):
        ...
        if raw_predictions:
            video_info = {
                "fps": (
                    raw_predictions[-1]["frame_idx"] / raw_predictions[-1]["timestamp"]
                    if raw_predictions[-1]["timestamp"] > 0
                    else 30.0
                ),
    ...
    events_data = {
        ...
        "config": {
            ...
            "marker_buffer": (
                config.get("mlt", {}).get("marker_buffer", 2.0) if config else 2.0
            ),
        },
    ...
    if output_file is None:
        output_file = predictions_file.replace("/predictions/", "/events/").replace(
            "_predictions.json", "_events.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        help="Output events JSON path (default: auto-generate from predictions path)",
    )
    ...
    threshold = (
        ...
        else config["inference"]["threshold"]
    )
    min_duration = (
        args.min_duration
        if args.min_duration is not None
        else config["inference"].get("min_event_duration", 0.0)
    )
