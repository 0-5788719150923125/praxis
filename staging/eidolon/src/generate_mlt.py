        ...
        if not merged:
            ...
            if current_clip["start"] <= previous_clip["end"]:
                ...
                previous_clip["duration"] = (
                    previous_clip["end"] - previous_clip["start"]
                )

                # Track which events were merged
                previous_clip["event_ids"].append(
                    current_clip.get("event_id", len(merged) + 1)
                )

                # Log the merge
                overlap = original_end - current_clip["start"]
                print(
                    f"  ↳ Merged overlapping clips (overlap: {overlap:.2f}s) - "
...
def create_mlt_project(
    ...
    fps: float,
    output_path: str,
    marker_buffer: float = 2.0,
    post_buffer: float = 1.0,
    mode: str = "cut_markers",
    mute_audio: bool = False,
    add_benny_hill: bool = False,
    ...
    print(
        f"  Format: {'Vertical (1080x1920, zoomed)' if vertical_format else 'Horizontal (from video)'}"
    )
    print()
    ...
    black = etree.SubElement(
        mlt, "producer", id="black", **{"in": "00:00:00.000", "out": video_duration_tc}
    )
    ...
    if mode == "cut_markers":
        ...
        for event in events:
            ...
            print(
                f"  Raw cut at {format_timecode(cut_time, fps)} (event at {format_timecode(event['start_time'], fps)})"
            )
        ...
        for cut_time in raw_cut_points:
            if min_cut_time <= cut_time <= max_cut_time:
                ...
                print(
                    f"  Skipped cut at {format_timecode(cut_time, fps)} (too close to video boundary)"
                )
        ...
        if cut_points:
            ...
            for i, (start, end) in enumerate(zip(segment_starts, segment_ends)):
                ...
                etree.SubElement(chain, "property", name="mlt_service").text = (
                    "avformat-novalidate"
                )
                etree.SubElement(chain, "property", name="shotcut:caption").text = (
                    video_name
                )
            ...
            entry = etree.SubElement(
                playlist0, "entry", producer=chain_id, **{"in": start_tc, "out": end_tc}
            )
        ...
        for i, event in enumerate(events):
            ...
            print(
                f"  Event {i+1}: {format_timecode(clip_start, fps)} → {format_timecode(clip_end, fps)} (duration: {clip_duration:.2f}s)"
            )
        ...
        if len(merged_clips) < len(clips):
            print(
                f"✓ Reduced {len(clips)} clips to {len(merged_clips)} clips after merging"
            )
        else:
        ...
        for chain_index, clip in enumerate(merged_clips):
            ...
            if len(event_ids) > 1:
                print(
                    f"  Clip {chain_index+1} (merged): Events {event_ids} → {format_timecode(clip_start, fps)} - {format_timecode(clip_end, fps)} ({clip_duration:.2f}s)"
                )
            else:
                print(
                    f"  Clip {chain_index+1}: Event {event_ids[0]} → {format_timecode(clip_start, fps)} - {format_timecode(clip_end, fps)} ({clip_duration:.2f}s)"
                )
    ...
    if vertical_format:
        affine_filter = etree.SubElement(
            playlist0, "filter", id="filter0", out=total_duration_tc
        )
        etree.SubElement(affine_filter, "property", name="background").text = (
            "color:#00000000"
        )
        etree.SubElement(affine_filter, "property", name="mlt_service").text = "affine"
        etree.SubElement(affine_filter, "property", name="shotcut:filter").text = (
            "affineSizePosition"
        )
        etree.SubElement(
            affine_filter, "property", name="transition.fix_rotate_x"
        ).text = "0"
        etree.SubElement(affine_filter, "property", name="transition.fill").text = "1"
        etree.SubElement(affine_filter, "property", name="transition.distort").text = (
            "0"
        )
        etree.SubElement(affine_filter, "property", name="transition.rect").text = (
            "-1122.45 -1998.5 3328.56 5917 1"
        )
        etree.SubElement(affine_filter, "property", name="transition.valign").text = (
            "middle"
        )
        etree.SubElement(affine_filter, "property", name="transition.halign").text = (
            "center"
        )
        etree.SubElement(affine_filter, "property", name="shotcut:animIn").text = (
            "00:00:00.000"
        )
        etree.SubElement(affine_filter, "property", name="shotcut:animOut").text = (
            "00:00:00.000"
        )
        etree.SubElement(affine_filter, "property", name="transition.threads").text = (
            "0"
        )
    ...
    if add_benny_hill:
        ...
        benny_hill_path = os.path.join(
            script_dir, "..", "static", "Benny-hill-theme.mp3"
        )
        ...
        audio_chain = etree.SubElement(
            mlt, "chain", id="chain-benny-hill", out=format_timecode(max_duration, fps)
        )
        etree.SubElement(audio_chain, "property", name="length").text = format_timecode(
            max_duration, fps
        )
        etree.SubElement(audio_chain, "property", name="eof").text = "pause"
        etree.SubElement(audio_chain, "property", name="resource").text = (
            benny_hill_path
        )
        etree.SubElement(audio_chain, "property", name="mlt_service").text = (
            "avformat-novalidate"
        )
        etree.SubElement(audio_chain, "property", name="audio_index").text = "0"
        etree.SubElement(audio_chain, "property", name="video_index").text = (
            "-1"  # Audio only
        )
        etree.SubElement(audio_chain, "property", name="shotcut:caption").text = (
            "Benny Hill Theme"
        )

        # Add fade-out filter (volume with keyframes)
        fadeout_filter = etree.SubElement(
            audio_chain,
            "filter",
            id="filter-fadeout",
            out=format_timecode(max_duration, fps),
        )
        ...
        etree.SubElement(fadeout_filter, "property", name="level").text = (
            f"{fade_start_tc}=0;{fade_end_tc}=-60"
        )
        ...
        etree.SubElement(fadeout_filter, "property", name="shotcut:filter").text = (
            "fadeOutVolume"
        )
        etree.SubElement(fadeout_filter, "property", name="shotcut:animOut").text = (
            format_timecode(fade_duration, fps)
        )
    ...
    etree.SubElement(tractor, "property", name="shotcut:projectAudioChannels").text = (
        "2"
    )
    ...
    etree.SubElement(transition1, "property", name="mlt_service").text = (
        "frei0r.cairoblend"
    )
    ...
    if add_benny_hill:
        transition_audio = etree.SubElement(
            tractor, "transition", id="transition-audio-mix"
        )
    ...
    if mode == "cut_markers":
        print(
            f"Video split into {len(segment_starts)} segments at {len(cut_points)} cut points"
        )
        print(f"\nTo use:")
...
def generate_from_events_file(
    events_file: str,
    output_path: str = None,
    marker_buffer: float = None,
    post_buffer: float = None,
    mode: str = None,
    mute_audio: bool = None,
    add_benny_hill: bool = None,
    vertical_format: bool = None,
...
def main():
    ...
    parser.add_argument(
        "--output",
        help="Output MLT path (default: outputs/projects/<video>_project.mlt)",
    )
    parser.add_argument(
        "--marker-buffer",
        type=float,
        help="Pre-event buffer in seconds (default: from config)",
    )
    parser.add_argument(
        "--post-buffer",
        type=float,
        help="Post-event buffer in seconds (default: from config)",
    )
    parser.add_argument(
        "--mode",
        choices=["cut_markers", "extract_clips"],
        default="cut_markers",
    ...
    audio_group.add_argument(
        "--mute-audio",
        action="store_true",
        dest="mute_audio_flag",
        help="Mute source video audio",
    )
    audio_group.add_argument(
        "--no-mute-audio",
        action="store_true",
        dest="no_mute_audio_flag",
        help="Enable source video audio",
    )

    # Benny Hill theme song flag
    benny_group = parser.add_mutually_exclusive_group()
    benny_group.add_argument(
        "--add-benny-hill",
        action="store_true",
        dest="add_benny_hill_flag",
        help="Add Benny Hill theme song",
    )
    benny_group.add_argument(
        "--no-benny-hill",
        action="store_true",
        dest="no_benny_hill_flag",
        help="Do not add Benny Hill theme song",
    )
    ...
    parser.add_argument(
        "--vertical-format",
        action="store_true",
        dest="vertical_format_flag",
        help="Use vertical 1080x1920 format with zoom",
    )
