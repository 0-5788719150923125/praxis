...
static func bake(song_path: String, fps: int, band_count: int,
	...
	var pos := 0
	var next_log := 0.25
	print("vortex bake: analyzing %.1fs of audio (%d frames)…" % [float(n) / RATE, total])
	while pos < n:
		var done := float(frames.size()) / total
		if done >= next_log:
			print("vortex bake: %d%%" % int(next_log * 100))
			next_log += 0.25
		if frames.size() % 16 == 0:
			write_progress(done)
...
static func _load_samples(song_path: String) -> PackedFloat32Array:
	...
	var tmp := ProjectSettings.globalize_path("user://vortex_bake.f32")
	var out := []
	var code := OS.execute("ffmpeg",
		["-y", "-loglevel", "error", "-i", src, "-ac", "1", "-ar", str(RATE), "-f", "f32le", tmp],
		out, true)
	if code != 0 or not FileAccess.file_exists(tmp):
		push_warning("vortex bake: ffmpeg could not decode %s (is ffmpeg on PATH?)" % src)
