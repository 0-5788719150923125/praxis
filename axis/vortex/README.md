...
- **Two ways to drive the show.** *Auto* is the above: the Director chooses and cuts on the music. *Manual* plays a **storyboard** - a user-authored linear sequence of scenes (`storyboards/*.json`), each entry naming its scene, behavior, shot, and an exit rule (a fixed `hold`, a musical `beat`/`movement`/`lull` cue, or the scene's own lifecycle). The storyboard is the first rung of orchestrating a whole visualization by hand; the data spec is meant to grow toward a full manual editor (see `storyboards/README.md`).
...
- **`eye`** - a single floating eyeball in the void (`EyeBody`): no lids, no blink. A shaded sclera with an iris (dark limbal ring + radial fibres + graded body), a pupil that dilates with energy, and a wet catchlight. It **looks around in centre-preferring saccades** - holding a fixation, then jumping (fast, jerky) to a new target drawn with a squared-radius centre bias, so it favours the neutral forward gaze. Declares `morph_out = "eye"`.
- **`two_eyes`** - two independent `EyeBody` eyes side by side, each looking around on its own. Declares `morph_in = "eye"`: arriving from `eye` it plays the **split** (starts as one centred eye at the previous size, eases apart into two as it shrinks to pair size); entered by a plain cut it simply opens already split.
...
- [ ] **Scene-spec pipeline** (the north star): a declarative spec that *samples a configuration* of geometry families + modifiers + materials + motion + lighting and composes them, so lifelike scenes emerge from randomly integrating many domains rather than from hand-crafted scene code. `rocks` already samples a small material/geometry spec; generalize that into a shared sampler across families.
...
Working framework, live path end to end: import a song on the splash (or boot a CLI flag), watch seeded scenes react and change *with the music* - or hand-orchestrate them with a storyboard. Geometry, motion (behaviors), lifecycle, exit cue, **and render kind** are independent typed axes; the physics is a composable **primitive registry**; and there is now a real **forced-perspective 3D path** (`Lens3D` / `Scene3D` / `Plane3D`) that the 2D scenes are migrating onto, with generated geometry (procedural rocks) as the first proof. An in-app **feedback console** (`` ` ``) captures a scene record + screenshot for authoring. Bake/Movie-Maker export, more scenes on the 3D path, and richer storyboards are next. Standalone; it does not import or depend on Praxis.
