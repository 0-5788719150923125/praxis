...
**Two pages face you, like an open book.** That is not decoration - it is what makes the framing work. A portrait sheet inside a landscape frame cannot cover it, so a single page forced the camera in until the page stopped reading as a page and still left a wedge of desk in shot; a spread is about 1.33 wide against the frame's 1.78, so the paper covers the picture at a shot that shows several panels at once. It also gives a pan the length of two pages to travel across instead of one, and it gives the page turn a real hinge: the right-hand leaf swings on the spine, and the page printed on its back is the next left-hand page, landing exactly where it is about to be drawn flat.

**A shot arrives before it is cut.** The camera asks the Director how long the current scene is likely to have left and sizes the move to land with time to spare, then settles into the remainder. Before this the two timings were unrelated - on a driving passage fewer than one move in twenty could physically finish, so the camera was permanently mid-convergence when the cut arrived.

**Footage is treated as a special case.** A video panel opens the spread when there is one, and it is framed to show the *whole* video rather than a crop of it - every other panel is fitted on height alone and allowed to overflow the sides, which suits an abstract field and does not suit a face.

**A spread is read a few panels at a time.** The camera settles on three to five moments, spread across the sheet, and then the leaf turns - it does not stop on every panel before turning, and it never rests on the same panel twice. The rest of the spread is drawn the whole time and every pan crosses it; being passed over is not the same as being skipped.

**One slider runs the camera.** _Camera_ in the Generative panel goes from 0 - a slow drift that barely turns and never cuts - through 1, the tuned default, to 2: fast, restless, cinematic, with real jump cuts. It is one knob over the whole behaviour rather than a setting per symptom, scaling how far a shot may swing, how many shots that swing is spread over, how long a move lasts, how much of the vocabulary is discontinuous and how deep a push goes. Rotation is also a **bounded walk** rather than a fresh draw each cut, so a big change of angle arrives over three or four shots instead of one.

**The camera never stops, and it is never tight on nothing.** Two rules do that. A move that runs out before the next cut arrives immediately plans its successor from wherever it ended, so there is no frozen picture waiting on the Director - which there used to be, for a median of twelve seconds a shot. And the zoom is coupled to the content under the aim: dead centre on a panel the shot is as tight as the move asked for, and out over a gutter it opens automatically to a shot of the page. The camera physically cannot end up close on the corner of a panel.
