...
CLI_FLAGS: List[Tuple[str, str, str, bool]] = [
    ...
    (
        "--scene",
        "<name|N>",
        "Pin one scene for authoring (by script-name substring or registry "
        "index).",
        False,
    ),
    (
    ...
    (
        "--use-bake",
        "",
        "Drive `Spectrum` from the song's cached bake instead of the live "
        "analyzer.",
        True,
    ),
    (
...
def _parse_registry_dict(text: str, const_name: str = "REGISTRY") -> List[Tuple[str, str]]:
...
class SceneInfo:
    def __init__(self, script: Script) -> None:
        ...
        self.render_kind = kind.group(1) if kind else (
            "scene3d" if script.extends == "Scene3D" else "canvas"
        )
...
def _collect_scenes() -> Tuple[Dict[str, SceneInfo], List[str]]:
    ...
    scenes = {
        p.stem: SceneInfo(Script(p)) for p in sorted(SCENES_DIR.glob("*.gd"))
    }
...
def _render_readme_scenes_block(
    scenes: Dict[str, SceneInfo], groups: List[str]
) -> str:
    lines = [
...
def _render_cli_doc(found: Dict[str, List[str]]) -> str:
    ...
    for flag in undocumented:
        warn(f"CLI flag {flag} ({', '.join(found[flag])}) missing from CLI_FLAGS in docs.py")
...
def _render_index(
    ...
    for title, desc, files in SCRIPT_GROUPS:
        ...
        lines.append("")
    stray = sorted(
        p.name for p in SCRIPTS.glob("*.gd") if p.name not in grouped
    )
    if stray:
        warn(
            "scripts not in docs.py SCRIPT_GROUPS: "
            + ", ".join(stray)
        )
        lines.extend(
            [
...
def main() -> int:
    ...
    pages = [
        ("scenes", "Scene catalogue", "every visualizer scene, from its own doc comment."),
