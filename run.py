...
class PraxisTrainer(LightningModule):
    ...
    def __init__(self, model, optimizer, scheduler, hparams):
        ...
        if isinstance(hparams, dict):
            ...
            hparams_obj = SimpleNamespace(**hparams)
        else:
            hparams_obj = hparams
        self.model = self._try_compile_model(model, hparams_obj)
        
        # Try to compile the optimizer as well
        self.optimizer = self._try_compile_optimizer(optimizer, hparams_obj)

    def _try_compile_model(self, model, hparams):
        """
    ...
    def _try_compile_optimizer(self, optimizer, hparams):
        """
