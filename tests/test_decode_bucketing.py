...
def test_no_mask_is_synthesized():
    """A caller passing no mask keeps passing none, so the bucketed and
    unbucketed calls differ in exactly one thing (the length). What makes the
    pad inert is causality, not the mask."""
    ids = torch.zeros(1, 10, dtype=torch.long)
    with decode_buckets(rungs=(64,)):
        _, mask, _ = pad_for_decode(ids, None)
    assert mask is None
