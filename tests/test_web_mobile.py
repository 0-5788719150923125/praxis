...
def test_mobile_viewport_no_horizontal_overflow(page: Page):
    ...
    assert (
        body_scroll_width <= viewport_width + 5
    ), f"Body scrollWidth {body_scroll_width} exceeds viewport {viewport_width}"
    ...
    for tab_id in tabs:
        ...
        if tab_button.count() > 0:
            ...
            tab_container = page.locator(
                f"#{tab_id}-container, .{tab_id}-container"
            ).first
            if tab_container.count() > 0:
                ...
                if box:
                    ...
                    assert (
                        right_edge <= viewport_width
                    ), f"Tab {tab_id} overflows: right edge {right_edge} > viewport {viewport_width}"
...
def test_mobile_input_field_visible(page: Page):
    ...
    assert (
        right_edge <= viewport_width
    ), f"Input field overflows: right edge {right_edge} > viewport {viewport_width}"
    ...
    if research_button.count() > 0:
        ...
        for i in range(min(count, 3)):  # Check first 3 charts
            ...
            if box:
                right_edge = box["x"] + box["width"]
                assert (
                    right_edge <= viewport_width + 5
                ), f"Chart card {i} overflows: {right_edge} > {viewport_width}"
    ...
    if spec_button.count() > 0:
        ...
        for i in range(min(count, 3)):
            ...
            if box:
                ...
                assert (
                    right_edge <= viewport_width + 5
                ), f"Code block {i} overflows: {right_edge} > {viewport_width}"
