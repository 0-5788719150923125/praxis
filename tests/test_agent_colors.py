    ...
    if agents_button.count() > 0:
        ...
        for msg in console_messages:
            if msg.startswith("[COLOR] Agent:"):
                ...
                # Parse final color
                current_agent["final_color"] = msg.replace(
                    "[COLOR]   Final:", ""
                ).strip()
        ...
        if agents_data:
            ...
            for freshness, hashes in freshness_values.items():
                if len(hashes) > 1:
                    print(
                        f"  ⚠ Multiple commits with same freshness {freshness}: {', '.join(hashes)}"
                    )
        else:
        ...
        assert (
            len(console_messages) > 0
        ), "No console messages captured - debugging may not be working"
