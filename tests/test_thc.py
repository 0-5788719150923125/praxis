...
class TestTemporalHealthComplex:
    ...
    def test_gate_initialization_strategies(self):
        ...
        assert torch.allclose(thc_zeros.gate.weight, torch.zeros_like(thc_zeros.gate.weight))
        assert torch.allclose(thc_zeros.gate.bias, torch.zeros_like(thc_zeros.gate.bias))

        # Test small initialization
        thc_small = TemporalHealthComplex(d_model=128, gate_init="small")
        assert not torch.allclose(thc_small.gate.weight, torch.zeros_like(thc_small.gate.weight))
        assert torch.allclose(thc_small.gate.bias, torch.zeros_like(thc_small.gate.bias))

        # Test ones initialization
        thc_ones = TemporalHealthComplex(d_model=128, gate_init="ones")
        assert torch.allclose(thc_ones.gate.weight, torch.zeros_like(thc_ones.gate.weight))
    ...
    def test_complex_representations(self, thc_module, test_input):
        ...
        expected_shape = (test_input.shape[0], test_input.shape[1], thc_module.d_complex)
    ...
    def test_phase_statistics(self, thc_module, test_input):
        ...
        expected_keys = {
            "mean_magnitude", "magnitude_std", "mean_phase", "phase_std",
            "mean_phase_diff", "phase_diff_std"
        }
    ...
    def test_residual_connection_strength(self):
        ...
        with torch.no_grad():
            ...
            assert difference < 0.3, f"Output too different from input with zero gate: {difference}"
...
class TestTHCIntegration:
    ...
    def test_transformer_block_with_thc(self, config):
        ...
        # Use float mask instead of bool
    ...
    def test_parameter_count_increase(self, config):
        ...
        params_without = sum(p.numel() for p in block_without_thc.parameters() 
        ...
        assert increase_ratio < 0.3, f"Parameter increase too large: {increase_ratio:.2%}"
...
class TestTHCGeneralization:
    ...
    def test_phase_pattern_learning(self):
        ...
        config = PraxisConfig(hidden_size=128, num_heads=4, depth=2, dropout=0.1, vocab_size=50)
    ...
    def test_sequence_length_generalization(self):
        ...
        config = PraxisConfig(hidden_size=64, num_heads=2, depth=1, dropout=0.0, vocab_size=30)
    ...
    def test_thc_complex_statistics_evolution(self):
        ...
        config = PraxisConfig(hidden_size=128, num_heads=4, depth=1, dropout=0.1, vocab_size=50)
        model = TransformerBlock(config)
        
        # Get initial statistics
        test_input = torch.randn(4, 16, config.hidden_size)
        initial_stats = model.thc.get_phase_statistics(test_input)
        ...
        for _ in range(10):
            ...
            attention_mask = torch.triu(torch.ones(16, 16), diagonal=1)
            
            output, _, _, aux_loss = model(inputs, attention_mask)
            loss = output.sum() + aux_loss
        ...
        model.eval()
        with torch.no_grad():
            final_stats = model.thc.get_phase_statistics(test_input)
