"""ArcMixture, the cyclic mixture-of-depths router (praxis/routers/arc.py).
...
class TestArcMixtureLayout:
    ...
    def test_capacities_are_sized_to_num_layers_and_indexed_by_modulo(self):
        router = ArcMixture(make_config(num_layers=3))
...
class TestArcMixturePassBias:
    ...
    def test_delta_tables_sized_and_zero_init(self):
        """The per-pass delta is the only calibration (the base Linear's scalar
        bias is dropped), and it is exactly zero at init, so the logits start
        at the base router's."""
        config = make_config(depth=9, num_layers=3)
        router = ArcMixture(config)
        ...
        assert torch.all(router.delta_coef.weight == 0)
...
class TestArcMixtureMetrics:
    ...
    def test_specialization_is_zero_at_init(self):
        router = ArcMixture(make_config())
        # Zero-init -> identical pass rows -> no specialization.
