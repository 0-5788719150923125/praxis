...
class TestSSMBlock:
    ...
    def test_forward_pass(self, config):
        ...
        assert new_state.shape == (batch_size, config.hidden_size * config.ssm_expand_factor, config.ssm_state_size)
    ...
    def test_different_sequence_lengths(self, config):
        ...
        for seq_len in [1, 8, 64, 128]:
            ...
            assert output.shape == (batch_size, seq_len, config.hidden_size)
            assert state.shape == (batch_size, config.hidden_size * config.ssm_expand_factor, config.ssm_state_size)
