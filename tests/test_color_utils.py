...
def test_step_based_freshness():
    ...
    commits = [
        ...
        {'hash': 'dd83544', 'timestamp': 1761947377},  # newest (only 13 min after previous!)
    ]
    ...
    for commit in commits:
        ...
        freshness = position / (len(unique_timestamps) - 1) if len(unique_timestamps) > 1 else 1.0

        print(f"  {commit['hash']}: position {position}/{len(unique_timestamps)-1} → freshness = {freshness:.3f}")
    ...
    assert middle_freshness == 0.5, f"Middle commit should be 0.5, got {middle_freshness}"
...
def test_color_interpolation():
    ...
    print(f"Greyscale version: rgb({archived_grey[0]}, {archived_grey[1]}, {archived_grey[2]})")
    ...
    assert full_color_online == primary_color, f"Freshness 1.0 should return original color, got {full_color_online} != {primary_color}"

    full_color_archived = lerp_color(archived_grey, archived_color_dark, 1.0)
    assert full_color_archived == archived_color_dark, f"Freshness 1.0 should return original archived color, got {full_color_archived} != {archived_color_dark}"
