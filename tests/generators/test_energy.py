"""praxis/generators/energy.py: EnergyGenerator sampling and LinearPrior - the streaming
ridge solve and the post-freeze re-solve (milestone-gated, damped,
reject-if-worse)."""

import torch

from praxis.generators.energy import (
...
# ── EnergyGenerator and the linear prior's solve ───────────────────────────────


def test_energy_generator_shapes():
    generator = EnergyGenerator(
        cond_dim=32, noise_dim=16, latent_dim=8, hidden_dim=32, num_blocks=2
    )
    h = torch.randn(3, 5, 32)
    samples = generator.sample(h, num_samples=4)
