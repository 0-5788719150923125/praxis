...
class TestSyntaxesAttention:
    ...
    def test_initialization(self, syntaxes_config):
        ...
        assert attention.context_size == getattr(
            syntaxes_config, "syntaxes_context_size", 128
        )
    ...
    def test_gradient_flow(self, syntaxes_config):
        ...
        inputs = torch.randn(
            batch_size, seq_len, syntaxes_config.hidden_size, requires_grad=True
        )
    ...
    def test_different_num_selected(self, base_config):
        ...
        for i in range(len(values)):
            for j in range(i + 1, len(values)):
                ...
                assert (
                    diff > 1e-4
                ), f"Outputs too similar between compression_ratio={val1} and {val2}: {diff}"
    ...
    def test_device_compatibility(self, base_config, device):
        ...
        inputs = torch.randn(
            batch_size, seq_len, base_config.hidden_size, device=device
        )
