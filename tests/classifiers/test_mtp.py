"""praxis/classifiers/mtp: MultiTokenPrediction - the depth banks, prompt masking,
...
from praxis.classifiers.mtp import (
...
@pytest.fixture
def spec_config():
    """Byte-latent + prismatic4 classifier + dual memory + VEAR MTP: the
    drafting stack."""
    return PraxisConfig(
        vocab_size=1024,
        hidden_size=32,
...
def test_mtp_honors_the_prompt_mask(spec_config):
    """MTP takes UNDETACHED hidden states and the SHARED classifier, so an unweighted
    ...
    unmasked = mtp(
        mtp.prepare_inputs(hidden, ids, None, model.embeds, model.classifier)
    )
    masked = mtp(
        mtp.prepare_inputs(
            hidden, ids, None, model.embeds, model.classifier, loss_weights=mask
        )
    )
    ...
    all_on = mtp(
        mtp.prepare_inputs(
            hidden, ids, None, model.embeds, model.classifier, loss_weights=ones
        )
    )
    ...
    none_on = mtp(
        mtp.prepare_inputs(
            hidden, ids, None, model.embeds, model.classifier, loss_weights=zeros
        )
    )
...
def test_serpent_rnn_mtp_bank(spec_config):
    ...
    ids = torch.randint(4, 260, (2, 24))
    hidden = torch.randn(2, 24, spec_config.embed_size)
    inputs = mtp.prepare_inputs(hidden, ids, None, model.embeds, model.classifier)
    losses = mtp(inputs)
    ...
    with torch.no_grad():
        drafted = mtp.draft_next_tokens(
            hidden[:1, -1:, :], ids[:1, :1], model.embeds, model.classifier
        )
...
def test_per_depth_mtp_bank(spec_config):
    ...
    ids = torch.randint(4, 260, (2, 24))
    hidden = torch.randn(2, 24, spec_config.embed_size)
    losses = mtp(mtp.prepare_inputs(hidden, ids, None, model.embeds, model.classifier))
    assert torch.isfinite(losses.get_loss("mtp"))
...
# tests/inference/test_speculative.py.
