"""praxis/classifiers/harmonic.py: HarmonicField.
...
from praxis.classifiers.harmonic import (
    FAST_EPS,
    FAST_REPR_INTERVAL,
    FAST_SEGMENT,
    HarmonicField,
...
class _FakeCache:
    ...
    (including the current chunk, as at classifier time), plus the classifier
    side-store."""
...
def _chunked_equals_full(f, x, chunks):
    with torch.no_grad():
        ...
        for size in chunks:
            ...
            cache.length = (
                pos + size
            )  # trunk wrote this chunk before the classifier runs
...
def test_cached_decode_matches_full_forward(mode):
    ...
    forward. Before this the field evaluated every chunk at positions 0.. and,
...
def test_cached_decode_without_cache_is_full_recompute():
    ...
    sequence every call; the field must then behave exactly as untethered."""
...
@pytest.mark.parametrize("classifier_type", ["crystal_harmonic", "prismatic6"])
def test_harmonic_classifier_cached_logits_match(classifier_type):
    ...
    Under cached decode the classifier sees only the suffix, so without the
    classifier-side cache state it evaluated every chunk at positions 0.. and
    pooled the suffix alone. Prefill + one-token steps must match the full forward, logit for
    ...
    # The classifier actually left state behind (it is not falling back silently).
    assert any(k.startswith("HarmonicField:") for k in cache.classifier_states)
