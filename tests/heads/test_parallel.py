"""praxis/heads/parallel.py: ParallelHead and SurgicalParallelHead.
...
measurement every ParallelHead reports, and the per-arm training intervention
...
def test_transform_preserves_shape_and_gate_normalizes():
    ...
    assert head.transform(x).shape == x.shape
    assert head._gate_mean is not None and len(head._gate_mean) == 2
    assert abs(float(head._gate_mean.sum()) - 1) < 1e-5


def test_gate_is_learned_and_receives_gradient():
    head = _parallel()
    head.transform(torch.randn(2, 8, Cfg.hidden_size)).sum().backward()
...
def test_training_metrics_namespaced_with_gate():
    head = build("prismatic")
    logits = head(torch.randn(2, 8, Cfg.hidden_size))  # populates the gate stats
...
def test_prismatic_descriptions_namespaced_and_attributed():
    head = build("prismatic")
    stub = SimpleNamespace(head=head, reg=[], tasker=None, encoder=False)
...
def test_prismatic3_pure_arm_is_identity_at_init():
    head = build("prismatic3")
    x = torch.randn(2, 6, Cfg.hidden_size)
    assert head(x).shape == (2, 6, Cfg.vocab_size)
...
def test_gradient_reaches_the_crystal_centers():
    head = build("prismatic8")
    head(torch.randn(2, 5, Cfg.hidden_size)).sum().backward()
...
# ── The arm Jacobian, which every ParallelHead measures ────────────────────
...
def test_arm_conflict_reports_a_full_jacobian_on_a_plain_parallel_head():
    ...
    x, y = _batch()
    m = head.arm_conflict(x, y, _halo_objectives())
...
def test_arm_conflict_is_inert_at_eval_and_without_labels():
    head = build("prismatic8")
    x, y = _batch()
    assert head.eval().arm_conflict(x, y, _objectives()) == {}
    assert head.train().arm_conflict(x, None, _objectives()) == {}
...
def test_arm_conflict_does_not_disturb_the_training_graph():
    ...
    head = build("prismatic8").train()
    x, y = _batch()
    head.arm_conflict(x, y, _objectives())
    head(x).sum().backward()
...
def test_the_arm_row_uses_the_registered_cross_entropy():
    ...
    head = build("prismatic8").train()
    assert isinstance(head.objectives()["arm_ce"], CrossEntropyLoss)
    arm = head.branches[0]
    x, y = _batch()
    terms = Objectives()
...
def test_override_is_zero_when_arms_agree():
    ...
    out = ParallelHead._override_shares(
...
def test_equalizing_rows_gives_a_drowned_arm_a_vote_back():
    ...
    m = ParallelHead._override_shares(rows, norms, [0, 1, 2])
...
# ── SurgicalParallelHead (prismatic9): per-arm objectives, PCGrad trunk ────
#
...
def test_prismatic8_emits_no_arm_objectives():
    head = build("prismatic8").train()
    x, y = _batch()
    assert head.arm_objectives(x, y, _halo_objectives()) == {}
    assert head.arm_surgery is False


def test_prismatic9_emits_one_loss_per_arm_plus_the_surrogate():
    head = build("prismatic9").train()
    assert isinstance(head, SurgicalParallelHead)
    assert head.equalize_rows is True
    x, y = _batch()
    out = head.arm_objectives(x, y, _halo_objectives())
...
def test_surgery_refuses_rather_than_shipping_a_partial_jacobian():
    ...
    head = build("prismatic9").train()
    x, y = _batch()
    assert head.arm_objectives(x, y, _objectives()) == {}
...
def test_metrics_and_descriptions_line_up():
    ...
    for key in head.training_metrics():
...
def test_undetached_is_scoped_and_restores_the_surgery():
    ...
    never leave the head undetached, even when its body raises."""
    head = build("prismatic9").train()
