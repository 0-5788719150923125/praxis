...
class NeuralMemState(NamedTuple):
    ...
    momentum: Weights  # momentum accumulator, leading batch dim
...
class NeuralMemory(nn.Module):
    ...
    def __init__(
        ...
        # Data-dependent update hyperparameters: per-token lr, per-chunk
        # momentum and decay.
    ...
    def init_state(self, batch: int, device=None) -> NeuralMemState:
        ...
        momentum = {n: torch.zeros_like(w) for n, w in weights.items()}
        return NeuralMemState(0, weights, momentum)
    ...
    def forward(
        ...
        bn = b * num_chunks
        w0_rep = {k: v.repeat_interleave(num_chunks, dim=0) for k, v in weights.items()}
        ...
        # Momentum then weight-decay, each a parallel scan over the chunk axis.
        ...
        new_state = NeuralMemState(state.seq_index + n, new_weights, new_momentum)
