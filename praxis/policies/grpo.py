...
class GRPO(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.group_size = getattr(
            config, "grpo_group_size", 64
        )  # G: samples per question
        self.kl_coeff = getattr(
            config, "grpo_kl_coeff", 0.04
        )  # β: KL penalty coefficient
        self.clip_ratio = getattr(
            config, "grpo_clip_ratio", 0.2
        )  # ε: PPO-style clipping
    ...
    def compute_group_advantages(
        ...
        group_stds = torch.where(
            has_variance, group_stds, torch.tensor(min_std, device=group_stds.device)
        )
    ...
    def forward(
        ...
        if batch_size < self.group_size:
            ...
            if self.step_count == 0:
                print(
                    f"[GRPO] Batch size {batch_size} < group size {self.group_size}, using batch as single group"
                )
            ...
            if self.step_count == 0:
                print(
                    f"[GRPO] Batch size {batch_size} not divisible by {self.group_size}, using group size {actual_group_size}"
                )
        else:
        ...
        if torch.all(rewards == 0):
            if self.step_count % self.log_interval == 0:
                print(
                    f"[GRPO Info] All rewards are zero in this batch - skipping RL loss"
                )
        ...
        if self.step_count == 0:
            ...
            print(
                f"  advantages: min={advantages.min():.4f}, max={advantages.max():.4f}, mean={advantages.mean():.4f}"
            )
            print(
        ...
        if self.step_count == 0:
            ...
            print(
                f"  Expected size: {batch_size * actual_seq_len} = {batch_size} * {actual_seq_len}"
            )
        ...
        if torch.isnan(policy_term).any() or torch.isinf(policy_term).any():
            ...
            print(
                f"  seq_log_probs: min={seq_log_probs.min():.4f}, max={seq_log_probs.max():.4f}"
            )
            print(
        ...
        if self.step_count % self.log_interval == 0:
            with torch.no_grad():
                ...
                print(
                    f"  Advantages: mean={advantages.mean():.3f}, std={advantages.std():.3f}"
                )
                ...
                if actual_group_size < self.group_size:
                    print(
                        f"  Warning: Batch size should be divisible by {self.group_size}"
                    )
