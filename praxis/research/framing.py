...
from praxis.research.runs import experiment_name, experiment_stems
...
def _augment(config: Dict) -> Dict:
    """Add config-derived gate keys the raw YAML doesn't carry. ``recurrent_steps``
    is how many times the same physical layers are revisited: the decoder loops
    ``depth`` times over ``num_layers`` experts, so depth>num_layers means weights
    are reused across depth (recurrent mode, e.g. calm-d's 2 layers x 4 steps)."""
    depth, layers = config.get("depth"), config.get("num_layers")
    if isinstance(depth, int) and isinstance(layers, int) and 0 < layers < depth:
        config = {**config, "recurrent": True, "recurrent_steps": depth // layers}
    else:
        config = {**config, "recurrent": False}
    return config
...
def render(config: Dict) -> Tuple[str, Dict]:
    ...
    sections: Dict[str, List[Fragment]] = {}
    for frag in active:
        sections.setdefault(frag.section, []).append(frag)

    lines = ["% Generated by praxis/research/framing.py - do not edit by hand."]
