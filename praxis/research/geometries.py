...
Entry point: :func:`export_geometries`, driven by :mod:`praxis.research.build`.
"""
...
def export_geometries(limit: int = 4, scan: int = 40) -> dict:
    ...
    if not geometries:
        ...
        with open(OUT_TEX, "w") as fh:
            fh.write("% Generated by praxis/research/geometries.py - no crystal geometries found.\n"
                     "\\newcommand{\\paperGeometryFigure}{}\n")
        return {"count": 0, "panels": []}

    paths = [render_png(g, i + 1) for i, g in enumerate(geometries)]
    with open(OUT_TEX, "w") as fh:
        fh.write("% Generated by praxis/research/geometries.py - do not edit by hand.\n")
