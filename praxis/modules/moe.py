...
class RemoteMixtureOfExperts(nn.Module):
    ...
    def forward(self, input: torch.Tensor, *args: torch.Tensor, **kwargs: torch.Tensor):
        ...
        chosen_experts: List[List[RemoteExpert]] = (
            self.beam_search.batch_find_best_experts(
        ...
        masked_logits = torch.full(
            (1,), float("-inf"), device=expert_logits.device, dtype=expert_logits.dtype
        )
        ...
        averaged_outputs_flat = [
            (expert_weights[..., None] * tensor.flatten(start_dim=2))
            .view(tensor.shape)
            .sum(dim=1)
    ...
    def compute_expert_scores(
        ...
        expert_strides = torch.cumsum(
            torch.as_tensor([0] + expert_counts, device=device), dim=-1
        )[:-1]
        ...
        grid_indices = torch.zeros(
            [len(flat_experts), len(grid_scores)], dtype=torch.int64
        )
        ...
        scores = torch.full(
            (batch_size, max_num_experts), fill_value=-float("inf"), device=device
        )
        scores[flat_batch_indices, flat_local_indices] = (
            flat_scores  # backprop-able w.r.t. flat_scores
        )
        return scores
    ...
    def info(self):
        if self._expert_info is None:
            ...
            dummy_scores_concat = self.proj(
                torch.randn(1, self.proj.in_features, device=proj_device)
            )
            dummy_scores = (
                dummy_scores_concat.cpu()
                .detach()
                .split_with_sizes(self.beam_search.grid_size, dim=-1)
            )
            dummy_experts = self.beam_search.find_best_experts(
                dummy_scores, beam_size=1
            )
...
class _RemoteCallMany(torch.autograd.Function):
    ...
    def forward(
        ...
        *flat_inputs: torch.Tensor,
        ...
        for i in range(num_samples):
            for j, expert in enumerate(experts_per_sample[i]):
                ...
                new_task = RemoteExpertWorker.run_coroutine(
                    expert_forward(
                        expert.uid, flat_inputs_per_sample[i], serialized_tensors, stub
                    ),
                    return_future=True,
                )
        ...
        if not isinstance(info["outputs_schema"], tuple):
            ...
            outputs_schema = info["outputs_schema"]
        outputs = nested_map(
            lambda descriptor: descriptor.make_zeros(
                num_samples, max_experts, device=flat_inputs[0].device
            ),
            outputs_schema,
        )
        ...
        if len(responded_inds) > 0 or allow_zero_outputs:
            ...
            alive_flat_outputs_stacked = (
                torch.cat(outputs) for outputs in zip(*alive_flat_outputs)
            )
        ...
        mask = torch.zeros(
            [num_samples, max_experts], dtype=torch.bool, device=flat_inputs[0].device
        )
    ...
    @once_differentiable
    def backward(
        cls, ctx, *raw_grads: torch.Tensor
        ...
        inputs_per_expert = zip(
            *(tensor[alive_ii_cpu].split(1, dim=0) for tensor in flat_inputs_cpu)
        )
        grad_outputs_per_expert = zip(
            *(
                tensor[alive_ii_cpu, alive_jj_cpu].split(1, dim=0)
        ...
        for i, j, inputs_ij, grad_outputs_ij in zip(
            ...
            new_task = RemoteExpertWorker.run_coroutine(
                expert_backward(
                    expert.uid, inputs_and_grad_outputs, serialized_tensors, stub
                ),
                return_future=True,
            )
            pending_tasks[new_task] = (i, j)
        ...
        survivor_grad_inputs_stacked = (
            torch.cat(grad_inputs) for grad_inputs in zip(*survivor_grad_inputs)
        )
        # torch tensors, i-th tensor is of shape [num_backward_survivors, *flat_inputs_cpu[i].shape]

        grad_inputs = nested_map(
            lambda descr: descr.make_zeros(
                num_samples, device=flat_grad_outputs[0].device
            ),
        ...
        for grad_input, survivor_grad_stacked in zip(
            ...
            grad_input.copy_(
                grad_input_per_expert.to(flat_grad_outputs[0].device).sum(dim=1)
            )
    ...
    def _collect_responses(
        ...
        timeout_after_k_min = (
            float("inf") if timeout_after_k_min is None else timeout_after_k_min
        )
        num_successful_tasks = [0 for _ in range(num_samples)]
        pending_samples = (
            num_samples  # samples for which we have less than k_min results
        )
        ...
        try:
            ...
            for _ in range(len(task_to_indices)):
                timeout = (
                    max(0.0, t_finish - time.perf_counter())
                ...
                if task_output is not None:
                    ...
                    if num_successful_tasks[sample_index] == k_min:
                        ...
                        if (
                            ...
                            t_finish = min(
                                t_finish, time.perf_counter() + timeout_after_k_min
                            )
...
def _process_dispatched_task(
    task: Future, detect_anomalies: bool
