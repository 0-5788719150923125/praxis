...
from .moe import RemoteMixtureOfExperts, _RemoteCallMany
...
class RemoteSwitchMixtureOfExperts(RemoteMixtureOfExperts):
    ...
    def forward(self, input: torch.Tensor, *args: torch.Tensor, **kwargs: torch.Tensor):
        ...
        grid_softmax = [
            torch.softmax(grid_score, dim=-1) for grid_score in grid_scores_dropout
        ]
        ...
        self.grid_utilization = (
            self.utilization_alpha * self.grid_utilization
            + (1 - self.utilization_alpha) * batch_utilization
        )
        ...
        masked_probs = torch.zeros(
            (1,), device=expert_probs.device, dtype=expert_probs.dtype
        )
        ...
        averaged_outputs_flat = [
            (expert_probs[..., None] * tensor.flatten(start_dim=2))
            .view(tensor.shape)
            .sum(dim=1)
    ...
    def _compute_batch_utilization(self, batch_experts, expert_mask):
        batch_utilization = [
            torch.zeros(
                (dim_size,),
                dtype=self.grid_utilization.dtype,
    ...
    def compute_expert_scores(
        ...
        grid_indices = torch.zeros(
            [len(flat_experts), len(grid_probs)], dtype=torch.int64
        )
