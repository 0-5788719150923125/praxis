        ...
        for module_dir in self.staging_dir.iterdir():
            if module_dir.is_dir() and (module_dir / "module.yaml").exists():
                try:
                    ...
                    print(
                        f"[Modules] Found module: {manifest['name']} v{manifest.get('version', 'unknown')}"
                    )
                except Exception as e:
                    print(
                        f"[Modules] Warning: Failed to load module manifest {module_dir}: {e}"
                    )
    ...
    def load_module(
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                f"staging_{module_name}", init_file
            )
