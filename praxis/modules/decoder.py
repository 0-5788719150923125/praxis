...
class PraxisDecoder(nn.Module):
    ...
    def __init__(self, config: PraxisConfig):
        ...
        if config.hivemind:
            ...
            for i in range(config.num_layers):
                expert_name = self._generate_unique_pet_name(k=3)
    ...
    def _generate_pet_name(self, k):
        return random.choice(PREFIXES[:k]) + "~" + random.choice(SUFFIXES[:k]) + ".0"

    def _generate_unique_pet_name(self, k):
        new_name = self._generate_pet_name(k)
        if new_name in self.experts:
            return self._generate_unique_pet_name(k)
        else:
            return new_name

    def _search_for_experts(self, chance=0.1):
        if random.random() < chance:
            search_for = self._generate_pet_name(k=3)
            new_expert = get_experts(self.dht, [search_for])[0]
