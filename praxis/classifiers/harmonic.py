"""Harmonic classifier: 2D irrational-rotation field, multiplicatively coupled.
...
forces the field into the gradient path - the upstream cannot cancel by emitting
``h(x) - b`` because the bias scales features rather than adding to them.
The scorer is learnable - a frozen kernel projection cannot align with
...
class HarmonicField(nn.Module):
    ...
    metric_descriptions = {
        ...
        "harmonic_grad_ratio": {
            "description": (
                "||grad(amplitudes)|| / ||grad(scorer)||. Vanishing = learning is "
    ...
    def _phase_table(self, seq_len: int, device: torch.device, offset: int = 0):
        ...
        offset is the cached-decode continuation: the trunk hands the classifier
        only the new suffix, and the field is anchored to ABSOLUTE position.
        """
    ...
    # classifier call (modeling._bind_classifier_cache) and unbinds it after;
    # training and any cache-less forward see None. See
    # ``praxis.classifiers.base.decode_context``.
...
class HarmonicClassifier(BaseClassifier):
    ...
    def __init__(
        ...
        self._downstream = None  # injectable downstream scorer (grad-ratio)
        ...
        # byte-level tokenizers) since that is what the classifier sees.
        max_positions = int(
        ...
        if dims is None:
            ...
            # nothing for this classifier to build.
            self.field = None
            self.scorer = None
            return
        ...
        # Transform-only stages (in a SequentialClassifier) skip the scorer - the
        # terminal stage classifies, so the vocab projection would be dead.
        if build_scorer:
    ...
    def transform(self, hidden_states: Tensor) -> Tensor:
        """The field modulation - this classifier's contribution as a
        non-terminal SequentialClassifier stage."""
    ...
    def forward(self, hidden_states: Tensor, **kwargs: Any) -> Tensor:
        ...
        return self.scorer(h) if self.scorer is not None else h

    def set_downstream(self, scorer: Optional[nn.Module]) -> None:
        """Point grad-ratio at the scorer this field actually feeds, when
        used transform-only in a SequentialClassifier (else it has none of its own).

        Held in a tuple so it is *not* registered as a submodule: the
        downstream is owned elsewhere (the terminal stage), and registering it
        would duplicate its params in our state_dict and leak its metric
        descriptions into this stage's module walk."""
        self._downstream = (scorer,) if scorer is not None else None
    ...
    def _downstream_scorer(self) -> Optional[nn.Module]:
        """The learnable projection the field feeds into: our own ``scorer``
        when terminal, else the injected downstream scorer."""
    ...
    def training_metrics(self) -> dict:
        ...
        # past it through the downstream scorer. Skip silently if
        # gradients aren't available yet (pre-first-step) or the scorer
        # exposes no readable weight tensor.
        amps_grad = amps.grad
        scorer_weight = _scorer_weight(self._downstream_scorer())
        scorer_grad = scorer_weight.grad if scorer_weight is not None else None
        if amps_grad is not None and scorer_grad is not None:
            scorer_norm = float(scorer_grad.detach().norm().item())
            if scorer_norm > 0:
                out["harmonic_grad_ratio"] = (
                    float(amps_grad.detach().norm().item()) / scorer_norm
                )
        return out


def _scorer_weight(mod: Optional[nn.Module]) -> Optional[Tensor]:
    """Primary weight tensor of a downstream scorer: ``weight`` for a
    Linear, ``centers`` for the crystal geometry."""
    if mod is None:
        return None
