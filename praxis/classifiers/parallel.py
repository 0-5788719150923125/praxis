"""ParallelClassifier: run standardized Praxis classifiers side by side, gate-combined.

Where :class:`~praxis.classifiers.sequential.SequentialClassifier` chains stages
(each stage's ``transform`` composes, the terminal classifies), ParallelClassifier
runs its branch classifiers on the *same* input and blends their ``transform``
outputs with a learned per-token softmax gate::
...
It works at two levels. As a non-terminal SequentialClassifier stage it blends
branch ``transform`` outputs (feature-level). As a terminal/top classifier it
blends the branches' ``forward`` outputs (logit-level) and is itself the model's
classifier. The ``prismatic`` profile uses the latter as a top-level split that
balances bias against variance per token::

    Parallel(arms=[Sequential(HarmonicField),
                   Sequential(HarmonicField, CrystalClassifier)])

- branch 0 = a harmonic field read out by a plain linear classifier (a strong
...
differently), so there is no single scorer for cut-CE - fine because crystal
forbids it, so prismatic trains on full logits. A centroid loss (HALO) instead
borrows the crystal arm's centers via ``scorer`` (see that property).

Branches are passed as *builders* (a classifier class or ``functools.partial``
over one), exactly like SequentialClassifier. Because two branches can share a
class (two ``HarmonicField``s emit identical metric keys), every branch's metrics,
...
#
# THAT TURNS THE CLASSIFIER INTO A GENUINE MULTI-TASK PROBLEM, which is where the
...
class ParallelClassifier(BaseClassifier):
    """Gate-combined parallel branches; a SequentialClassifier stage or the
    top classifier."""

    # A composed classifier ties via a self-tying branch (e.g. crystal), so the model
    # keeps it under tie_word_embeddings rather than swapping in TiedClassifier.
    self_ties = True
    ...
    # arm Jacobian; the INTERVENTION is opt-in and only SurgicalParallelClassifier
    ...
    def __init__(
        ...
        *,
        branches: List[ClassifierSpec],
        gate_repulsion: float = 0.0,
        stem: Optional[ClassifierSpec] = None,
    ) -> None:
        ...
        hidden states instead. HaloClassifier sets it: HALOLoss scores the trunk
        ...
        if not branches:
            raise ValueError("ParallelClassifier needs at least one branch.")
        built = [
            b if isinstance(b, BaseClassifier) else b(config, encoder=encoder)
            for b in branches
        ]
        self.branches = nn.ModuleList(built)
        self.stem: Optional[BaseClassifier] = (
            None
            if stem is None
            else (
                stem
                if isinstance(stem, BaseClassifier)
                else stem(config, encoder=encoder)
            )
        )
        ...
        # classifier-registry profile (e.g. prismatic3_repel), not a config flag.
        ...
        # See SurgicalParallelClassifier. False everywhere else, so prismatic2-8 are
        ...
        # nothing to gate - the classifier passes through (mirrors HarmonicClassifier).
    ...
    def objectives(self) -> dict:
        """This classifier's per-arm cross-entropy, for the model's Objectives
    ...
    def undetached(self):
        """Restore the ordinary gradient path through this classifier, temporarily.

        A surgical classifier detaches every arm in the blend and detaches the
        gate's input, so the mixture cross-entropy trains ONLY the gate - the arms
        have their own objectives instead. That is right for the main loss and
        wrong for every other consumer of the same classifier, because detaching
        an arm's OUTPUT severs the path back to the classifier's INPUT.

        MTP is that other consumer. It transforms the trunk's hidden states into
        draft states and classifies them with this classifier, so under the detached
        ...
        Callers that own their own objective and want the whole classifier
        trained by it wrap their use of the classifier in this.
        """
    ...
    def _gate_in(self, hidden_states: Tensor) -> Tensor:
        """What the gate projects. Detached under a surgical classifier so the gate's
    ...
    def _branch_input(
        self, branch: BaseClassifier, stemmed: Tensor, trunk: Tensor
    ) -> Tensor:
        """Stem output, unless the branch declares it must score trunk features
        (``reads_trunk``, set by HaloClassifier - see __init__)."""
        return trunk if getattr(branch, "reads_trunk", False) else stemmed

    def transform(self, hidden_states: Tensor) -> Tensor:
        """The gated mixture of branch transforms - this classifier's
        contribution as a non-terminal SequentialClassifier stage."""
    ...
    def forward(self, hidden_states: Tensor, **kwargs: Any) -> Tensor:
        """Standalone (terminal): gate-combine the branches' logit outputs
        ...
        Terminal ParallelClassifiers classify; a non-terminal one blends features via
    ...
    def _arm_grads(self, hidden_states: Tensor, labels: Tensor, objectives=None):
        ...
        reason this is affordable. Each backward crosses one small arm.
    ...
    def arm_conflict(
        ...
        posterior responsibility. This is the classifier's own Jacobian, one row per
    ...
    def arm_objectives(
        ...
        * ``arm{i}_loss`` - arm i's own objective (cross-entropy, or HALO's
          geometry for the HALO arm) on the detached branch input. Trains arm
          i's parameters as a standalone classifier and reaches the trunk not at
          all. This is what lets the crystal arm train the way a bare crystal
          classifier does, which is the condition its PCA
        ...
        * ``arm_surgery`` - the surrogate ``(z * g_hat).sum()``, whose gradient
        ...
        gate, which is why SurgicalParallelClassifier detaches every arm in the
        ...
        if not grads or self._arm_gap:
            ...
            # its objective (e.g. HaloClassifier without HALOLoss as the criterion).
            return {}
        out: Dict[str, Tensor] = {
        ...
        out: dict = {
            "arm_cos_min": {
                "description": (
                    ...
                    "gradients where they branch - the classifier's Jacobian. "
    ...
    def set_downstream(self, scorer: Optional[nn.Module]) -> None:
        """Point every branch's grad-ratio at the real downstream scorer."""
        for b in self.branches:
            if hasattr(b, "set_downstream"):
                b.set_downstream(scorer)
        # The stem feeds a MIXTURE of readouts, so no single scorer is "the"
        ...
        if self.stem is not None and hasattr(self.stem, "set_downstream"):
            self.stem.set_downstream(scorer)
        ...
        for i in range(len(self.branches)):
            out[f"gate_weight_{i}"] = {
                ...
                "chart": {
                    ...
                    "group": "parallel_classifier",
                    "group_order": 60,
                    "order": 10,
                    "series_group": "parallel_gate",
                    "series_label": f"branch {i}",
                },
                "caller": "ParallelClassifier",
            }
        out["gate_entropy"] = {
            "description": (
...
class SurgicalParallelClassifier(ParallelClassifier):
    """ParallelClassifier whose arms train on their own objectives, combined by PCGrad.

    Identical to ParallelClassifier in the forward pass and at inference: the same
    ...
    arms learn the whole task and the classifier becomes an ensemble of near-
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # starvation this classifier exists to remove would come straight back in
