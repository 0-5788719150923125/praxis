"""Crystal classifier: distance-based classifier (harmonic loss).
...
to ``-n`` directly. The classifier returns ``pseudo_logits = -n * log(d²)``,
...
from praxis.classifiers.base import BaseClassifier, decode_context

# Number of crystal geometries in the VEAR-merged bank for the prismatic4
# classifier. Baked (fixed, model-agnostic) per the tuning-free stance; override
# with a partial(CrystalVearClassifier, n_experts=...) in a classifier profile.
CRYSTAL_BANK_SIZE: int = 4

EPS: float = 1e-4

# Paper default for the mean-column-RMS centers regularizer (Baek et al.).
# The crystal classifier reads this when computing its centers_rms aux loss.
...
# (``prob + alpha/V``), matching the grow-crystals ``model_l2loss`` classifier.
...
def _pca_density_grid(
    ...
    crystal classifier passes its ``[V, D]`` centers).
...
class CrystalGeometry(nn.Module):
    """Distance-based scorer: vocabulary centers emitting pseudo-logits for CE."""
...
class CrystalClassifier(BaseClassifier):
    """Distance-based classifier (harmonic loss).

    In encoder-attached mode the classifier sizes its centers to match the
    encoder's output layout (so the distance computation lives in the
    ...
    def __init__(self, config: Any, encoder: Optional[nn.Module] = None) -> None:
        ...
        if config.loss_func == "cut_cross_entropy":
            raise ValueError(
                "classifier_type='crystal' is incompatible with "
        ...
        # states are hidden_size. Mirrors TiedClassifier.
        self.pre_projection: Optional[nn.Module] = None
        dims = self.output_dims()
        if dims is None:
            raise ValueError(
                "classifier_type='crystal' needs an encoder that declares an output "
    ...
    def dashboard_snapshots(self) -> dict:
        ...
        """
        grid = _pca_density_grid([self.scorer.centers])
    ...
    def aux_losses(self) -> dict:
        ...
        if lam <= 0.0:
            return {}
        c = self.scorer.centers
        rms = c.pow(2).mean(dim=0).clamp_min(1e-12).sqrt().mean()
        return {"centers_rms": lam * rms}


class CrystalVearClassifier(BaseClassifier):
    """A bank of ``CrystalGeometry``s merged by a VEAR router.

    Where ``CrystalClassifier`` learns one center geometry, this learns ``n_experts``
    and lets VEAR pick a discrete, per-context blend: sharpened routing selects a
    near-single crystal per position (not the smeared convex-hull average SMEAR would
    give), and VEAR's inter-expert repulsion keeps the geometries distinct - a
    "population" of output geometries. Drop-in for ``CrystalClassifier`` inside a
    prismatic arm (``prismatic4``). Reuses VEAR's merge machinery
    (``praxis/routers/vear.py``); see ``next/roadmap.md`` (geometry banks + voting).

    Training and inference both route per POSITION on the running mean of the
    prefix (``_route_causal``), so no position's crystal reads a later byte and
    no row reads another; ``_crystal_logits_perpos`` merges per position without
    materializing the centers.
    """
    ...
    # running sum is carried across chunks (praxis.classifiers.base.decode_context).
    ...
    def __init__(
        ...
        if config.loss_func == "cut_cross_entropy":
            raise ValueError(
                "CrystalVearClassifier (prismatic4) is incompatible with "
                "loss_func='cut_cross_entropy' (cut-CE assumes a dot-product classifier)"
            )
        # Deferred import: routers/ -> classifiers/ would otherwise risk an
        # import cycle.
        ...
        if dims is None:
            raise ValueError(
                "classifier_type='prismatic4' needs an encoder that declares an output "
                "layout; it can't pair with a loss-owning encoder (handles_loss)."
            )
        feature_dim, vocab_size = dims
        if self.has_encoder:
    ...
    def _expert_centers(self) -> Tensor:
        ...
        logits, or the diagnostics. ``CrystalSmearClassifier`` uses it to hold one
    ...
    def _merge_weights(self, hidden_states: Tensor, mask: Optional[Tensor]) -> Tensor:
        """Per-position merge weights ``[B, T, N]`` on the running mean of the
        prefix, so no position reads a later byte and no row reads another.
        Training draws expert dropout per example, shared by its positions."""
        probs = self._route_causal(hidden_states, mask)
        v = self.bank
        if self.training and v.dropout_rate > 0:
            keep = torch.bernoulli(
                torch.full(
                    (probs.shape[0], 1, probs.shape[-1]),
                    1.0 - v.dropout_rate,
                    device=probs.device,
                    dtype=probs.dtype,
                )
    ...
    def forward(self, hidden_states: Tensor, **kwargs: Any) -> Tensor:
        ...
        # Routing is PER POSITION on the prefix mean in training and inference
        # alike, so every position is a causal read: a batched forward equals
        # each sequence run alone (the padding invariance batched decode needs)
        # AND a long row equals each of its prefixes run alone (the property
        # single-row speculative verification needs). Inputs without a sequence
        # axis are length-1 sequences, one per row.
        rows = hidden_states.dim() < 3
        x = (
            hidden_states.reshape(-1, 1, hidden_states.shape[-1])
            if rows
            else hidden_states
        )
        sharp = self._merge_weights(x, None if rows else mask)  # [B, T, N]
        logits = self._crystal_logits_perpos(x, sharp, self.bank.experts[0])
        return (
            logits.reshape(*hidden_states.shape[:-1], logits.shape[-1])
            if rows
            else logits
        )
    ...
    def _crystal_logits(self, x: Tensor, centers: Tensor, ref: nn.Module) -> Tensor:
        """CrystalGeometry.forward, but with externally-merged ``centers`` (the
    ...
    def training_metrics(self) -> dict:
        ...
        # PCA cards and _bank_distinctness use: CrystalSmearClassifier's bank.experts
        # holds only the shared base, a table the merge never uses on its own.
    ...
    def dashboard_snapshots(self) -> dict:
        ...
        so a subclass that stores the bank differently (``CrystalSmearClassifier``:
        one shared trunk plus rank-r deviations) still gets one card per EXPERT.

        Every panel is drawn in one shared frame, since the set exists to be
        compared. The frame spans every expert's rows, so one expert moving
        redraws all four panels together - the price of a common axis.
        """
    ...
    def all_metric_descriptions(self) -> dict:
        # Start from the module-walk (the per-expert CrystalGeometry modules
        # contribute the scalar descriptions), then swap the single-centers PCA
        # for one card per expert and drop the grad-norm (not tracked for the
        # merged bank).
        ...
        out["crystal_bank_distinctness"] = {
            ...
            "caller": "CrystalVearClassifier",
        }
        return out


class CrystalSmearClassifier(CrystalVearClassifier):
    """prismatic7's bank: the parent's routing over a base plus deviations.

    The parent holds ``N`` independent ``[V, D]`` center sets; here there is ONE
    set plus ``N`` rank-r deviations, the same merge in SMEAR's basis
    (arXiv:2306.03745, ``base + sum_e w_e delta_e``). It is EXACTLY the base at
