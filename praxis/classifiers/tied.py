"""Tied classifier that reuses the input embedding weights."""
...
class TiedClassifier(BaseClassifier):
    """
    Classifier with tied weights.
    Instead of having a separate output projection, this classifier reuses
    ...
    def __init__(
        ...
        """
        Initialize the tied classifier.

        Args:
            config: Model configuration
            encoder: Optional encoder reference. TiedClassifier does not
        ...
        if self.has_encoder:
            raise ValueError(
                "TiedClassifier is not supported with an encoder; the "
    ...
    def scorer(self) -> nn.Module:
        """
        Get the scorer module for loss computation.
        ...
        return TiedProjection(self.embedding_weight, self.pre_projection)
