"""HALO classifier: hyperspherical distance classifier (the HALO scoring function).
...
This module restores that contract inside the standardized classifier stack:
``HaloGeometry`` owns the centroids, the learnable gamma temperature and
the abstain calibration, and emits the true distance logits
``-gamma * ||x - c||^2 / D`` (top-normalized to 0, matching the crystal
classifier's logit-scale contract for inference processors). ``HALOLoss``
delegates to this module when it finds it on the scorer path, so
...
direct ParallelClassifier branch, or the model's classifier). Behind a feature
transform (e.g. Sequential(HarmonicField, HaloClassifier)) the loss - which
...
class HaloClassifier(BaseClassifier):
    """Classifier emitting HALO distance logits; the honest HALO arm.
    ...
    Either way the arm STAYS. ParallelClassifier.scorer finds it by ``is_halo`` to put
    ...
    # Class-level default; the constructor may override per instance so a
    # classifier profile can choose without changing the ones already running.
    # See the class docstring; ParallelClassifier reads it in its terminal blend.
    detach_in_blend = True

    # HALOLoss scores the TRUNK embeddings, so this arm must score those same
    # features at inference. When a ParallelClassifier carries a shared stem, this
    ...
    def __init__(
        ...
        if config.loss_func == "cut_cross_entropy":
            raise ValueError(
                "HaloClassifier is incompatible with loss_func='cut_cross_entropy' "
                "(cut-CE assumes a dot-product classifier)"
            )
        # Instance attribute shadows the class default; ParallelClassifier's getattr
        ...
        if dims is None:
            raise ValueError(
                "HaloClassifier needs an output layout; it can't pair with a "
                "loss-owning encoder (handles_loss)."
            )
        feature_dim, vocab_size = dims
        self.scorer = HaloGeometry(hidden_size=feature_dim, vocab_size=vocab_size)

    def forward(self, hidden_states: Tensor, **kwargs: Any) -> Tensor:
        return self.scorer(hidden_states)

    def arm_loss(self, inp, labels, objectives=None):
        ...
        Returns None when the main criterion is not HALOLoss, so the caller
        drops the row rather than substituting something wrong.
        """
        main = getattr(objectives, "main", None)
        fn = getattr(main, "geometry_only", None)
        if fn is None:
            return None
        return fn(inp, labels, self.scorer)
