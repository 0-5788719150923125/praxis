...
Two execution paths are handled internally:
  ...
  Encoder (patch-level): patch embeds projected to embed_size, MSE loss
    vs target patch representations - owns the input and output projections.
"""
...
from praxis.classifiers.mtp.conv import ConvMTPModule
from praxis.classifiers.mtp.transformer import TransformerMTPModule
...
#
# A draft is not cheap here. Each pays a full classifier evaluation, ~5 ms against a
...
def _is_byte_latent(encoder_type) -> bool:
    ...
    encoders package pulls in blocks/classifiers, so a module-level import would
    cycle."""
    if not encoder_type:
        return False
...
class MTPInputs:
    ...
    # reaches the trunk and the SHARED classifier at positions the main CE
...
class MultiTokenPrediction(nn.Module):
    ...
    def __init__(self, config):
        ...
        # per full forward. A byte-latent model's shared classifier classifies the
        ...
        depth_config = config
        if self.byte_level:
            # Depths operate on the byte classifier's input width. Build them from a
        ...
        # per depth (praxis/classifiers/mtp/vear.py); serpent_rnn = one shared gated
        # serpent cell unrolled K times (praxis/classifiers/mtp/rnn.py); per_depth = K
        # independent light harmonic transforms, nothing shared
        # (praxis/classifiers/mtp/independent.py). Registry types: K independent
        # per-depth modules built from the ``mtp`` registry.
        self.is_vear = config.mtp_type == "vear"
        self.bank = None
        self.depths = None
        if self.is_vear:
            from praxis.classifiers.mtp.vear import VearHarmonicMTPBank

            self.bank = VearHarmonicMTPBank(depth_config, self.num_depths)
        elif config.mtp_type == "serpent_rnn":
            from praxis.classifiers.mtp.rnn import SerpentRNNMTPBank

            self.bank = SerpentRNNMTPBank(depth_config, self.num_depths)
        elif config.mtp_type == "per_depth":
            from praxis.classifiers.mtp.independent import PerDepthMTPBank
        ...
        # Non-byte encoder path (e.g. CALM) owns input and output projections
        # for patch-level MTP; byte-level and token paths reuse the model's
        # classifier.
        if self.encoder_path and not self.byte_level:
    ...
    def prepare_inputs(
        ...
        Args:
            ...
            classifier: The model's classifier (token path) - ignored on encoder path
        ...
        if self.byte_level:
            ...
            # the shared byte classifier classifies each drafted hidden. Same shape
            ...
            return MTPInputs(
                ...
                classifier=self.patch_projection,
                loss_fn=self.criterion,
                attention_mask=attention_mask,
                loss_weights=None,
            )
    ...
    def forward(self, inputs: MTPInputs):
        ...
        depths_computed = 0
        # Per-depth draft accuracy: how often the depth-k module predicts the true
        ...
        for k in range(self.num_depths):
            ...
            # Predictions via the classifier, trimmed for alignment
            preds = inputs.classifier(h_k)[:, :-1]
    ...
    def draft_next_tokens(
        self, hidden_state, first_token_id, embed_fn, classifier_fn, max_depths=None
    ):
        ...
        paying a classifier evaluation, so ``max_depths`` (default: ``draft_width``)
        ...
        Args:
            ...
            classifier_fn: Classifier module for computing logits
