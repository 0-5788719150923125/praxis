...
penalty the terminal relies on, the suppressed control ids the byte head must
...
def speculative_decoding(
    ...
    **model_kwargs: Dict[str, Any],
    ...
    single_row = bool(getattr(model.head, "causal_readout", False))
    ...
    while num_new < max_new_tokens:
        ...
        if h_row is None:
            ...
            last_logits = model.head(h_row)[:, -1, :]
        ...
        draft_ids = model.mtp.draft_next_tokens(
            h_row[:, -1:, :], token_0_2d, embed_fn, model.head
        )
