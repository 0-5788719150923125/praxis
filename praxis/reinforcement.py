...
class RLPolicy(nn.Module):
    ...
    def forward(
        ...
        if rewards is not None and self.training:
            ...
            policy_logits = (policy_hidden_states - hidden_states).pow(2).sum(dim=-1).mean(dim=1)
            policy_log_probs = -policy_logits  # Negative squared distance as log prob proxy
            ...
            if self.step_count % self.log_interval == 0:
                with torch.no_grad():
                    ...
                    print(f"  Rewards: mean={mean_reward:.3f}, min={rewards.min():.3f}, max={rewards.max():.3f}")
                    print(f"  Values: mean={mean_value:.3f}, min={values.min():.3f}, max={values.max():.3f}")
                    print(f"  Advantages: mean={mean_advantage:.3f}")
                    print(f"  Baseline: {self.baseline.item():.3f}")
                    print(f"  Losses: policy={policy_loss:.4f}, value={value_loss:.4f}, total={rl_loss:.4f}")
