"""Harmonic latent generator for CALM.

A drop-in sibling to ``FlowGenerator`` (same ``flow_loss`` / ``forward`` /
``sample`` / ``set_prior`` surface) selectable via the ``generators`` registry.
It trains by flow matching like ``FlowGenerator``, but the flow lives in a small
...
scales linearly): if the codec's latent is smooth/low-frequency, the generator
...
applied to the generator. ``num_freqs`` is the knob: fewer = stronger smoothness
prior + more compression (the real bet); ``2*num_freqs == latent_dim`` is the
lossless-reparameterization baseline (~equivalent to ``FlowGenerator``).

Reuses ``FlowGenerator``'s velocity net and integrator. The projection trick keeps
the encoder loss path identical: ``flow_loss`` accepts latent-space targets and
a latent-space ``x0`` (what the encoder builds for the flow generator) and projects
both into coefficient space internally, so the encoder routes ``generator_type
...
from praxis.generators.flow import FLOW_SAMPLE_STEPS, FLOW_SOLVER, FlowMLP
...
class HarmonicLatentGenerator(nn.Module):
    ...
    a latent-width start state, projected internally), matching ``FlowGenerator``.
    """
    ...
    def set_prior(self, prior) -> None:  # no-op: harmonic generator uses no prior
        self.prior = None
    ...
    def flow_loss(
        ...
        shared with ``FlowGenerator`` unchanged. The orthonormal projection of a
