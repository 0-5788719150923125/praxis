"""Flow-matching generator for CALM.

A drop-in sibling to ``EnergyGenerator`` (same constructor signature, same
``sample`` / ``forward`` / ``set_prior`` surface) selectable via
the ``generators`` registry. Where the energy generator trains an implicit
...
The "noise" the energy generator maps from is replaced by the flow's own start
...
class FlowGenerator(nn.Module):
    ...
    Same constructor kwargs as ``EnergyGenerator`` for registry interchange;
