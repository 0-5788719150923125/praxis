"""Energy generator for CALM.

MLP-based generator following the reference implementation's generative head
...
- The final projection is a small (LayerNorm → Linear → SiLU → Linear)
  block whose last Linear is zero-initialised, so the generator starts
...
class LinearPrior(nn.Module):
    ...
    The energy generator adds ``W phi`` to its proposal and spends its gradient
...
registry.declare(
    "energy_priors",
    doc=(
        "Closed-form priors for the CALM energy generator, chosen by the encoder "
        "profile's ``energy_prior``. The flow and harmonic latent generators take "
        "none."
    ),
    entries={
        "none": Entry(
            None,
            (
                "No prior: the energy generator predicts the next latent unaided. The "
                "paper-pure ablation."
            ),
        ),
        "linear": Entry(
            ...
            (
                ...
                "frozen; the energy generator learns only the residual. The default "
                "wherever the energy generator is used."
            ),
        ),
        "harmonic": Entry(
...
class EnergyGenerator(nn.Module):
    ...
    def __init__(
        ...
        # Zero-init the last projection: the generator starts predicting
        # near-zero latents and grows from there. Without this the generator
    ...
    def sample(
        ...
        scoring rule, so the generator learns to map this noise to whatever
        ...
        it as a temperature, so low T concentrates on the generator's best guess.
