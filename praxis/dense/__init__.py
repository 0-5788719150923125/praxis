...
registry.declare(
    "dense",
    title="Feedforward experts",
    doc=(
        (
            "How a block's feedforward path is realized: MLP, GLU, KAN, polynomial, "
            "scatter, PEER, ... Entries are feedforward structure only; the activation a "
            "feedforward gates through is set on ``--activation-type``, where the run's "
            "Arguments card shows it."
        )
    ),
    entries={
        ...
        "peer_glu": Entry(
            partial(ParameterEfficientExpertRetrieval, glu=True),
            (
                "PEER with gated experts: each retrieved expert becomes a GLU (``up_e "
                "* (act(x . gate_e) * (x . down_e))``) instead of a rank-1 projection. "
                "The third bank row per expert is paid for out of the expert count, "
                "not the parameter budget, so this trades bank breadth for per-expert "
