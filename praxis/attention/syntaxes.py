...
class SyntaxesAttention(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.compression_size = getattr(config, 'compression_size', 64)  # Target compressed size
        self.compression_method = getattr(config, 'compression_method', 'learnable_interpolation')  # learnable_interpolation, linear_interpolation, pooling
        self.scoring_method = getattr(config, 'scoring_method', 'norm')  # norm or learned (for backwards compat)
        self.selection_method = getattr(config, 'selection_method', 'interpolation')  # interpolation, sliding_window, top_k
        ...
        if self.compression_method == 'learnable_interpolation':
            ...
            self.interpolation_proj = nn.Linear(self.hidden_size, self.compression_size, bias=False)
            # Position embeddings for the compressed sequence
            self.compressed_pos_emb = nn.Parameter(torch.randn(1, self.compression_size, self.hidden_size) * 0.02)
    ...
    def _compress_sequence(self, x: Tensor) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        if self.selection_method == 'interpolation':
            ...
            selected_indices = torch.arange(start_idx, seq_len, device=x.device).unsqueeze(0).expand(batch_size, -1)
            batch_indices = torch.arange(batch_size, device=x.device).unsqueeze(1)
            selected_tokens = x[batch_indices, selected_indices]
            return selected_tokens, selected_indices
        else:
    ...
    def _interpolate_sequence(self, x: Tensor, target_length: int) -> Tuple[Tensor, None]:
        """Interpolate sequence to target length using different methods."""
        batch_size, seq_len, hidden_size = x.shape
        
        if self.compression_method == 'learnable_interpolation':
            # Use learnable interpolation weights
            # Project each token to target_length interpolation weights
            interp_weights = self.interpolation_proj(x)  # [batch, seq_len, target_length]
            interp_weights = F.softmax(interp_weights, dim=1)  # Normalize over sequence dimension
            
            # Weighted sum to create compressed representation
            compressed = torch.bmm(interp_weights.transpose(1, 2), x)  # [batch, target_length, hidden]
            ...
            compressed = torch.stack(compressed, dim=1)  # [batch, target_length, hidden]
            return compressed, None
            ...
            pooled = F.adaptive_avg_pool1d(x_reshaped, target_length)  # [batch, hidden, target_length]
            compressed = pooled.transpose(1, 2).contiguous()  # [batch, target_length, hidden]
            return compressed, None
            
        else:
            raise ValueError(f"Unknown compression method: {self.compression_method}")
    
    def _interpolate_to_full_sequence(self, 
                                    sparse_output: Tensor, 
                                    selected_indices: Tensor, 
                                    target_seq_len: int) -> Tensor:
        ...
        if self.selection_method == 'sliding_window':
            ...
            full_output = torch.zeros(batch_size, target_seq_len, hidden_size, device=device)
            
            # Get the starting position of the sliding window
            start_pos = selected_indices[0, 0].item()  # Same for all batches in sliding window
            ...
            positions = torch.arange(target_seq_len, device=device).unsqueeze(0).unsqueeze(-1).expand(batch_size, -1, -1)
    ...
    def _expand_compressed_sequence(self, compressed_output: Tensor, target_seq_len: int) -> Tensor:
        ...
        if self.compression_method == 'learnable_interpolation':
            ...
            full_expansion_weights = self.expansion_proj(compressed_output)  # [batch, compressed_len, max_seq_len]
            expansion_weights = full_expansion_weights[:, :, :target_seq_len]  # [batch, compressed_len, target_seq_len]
            expansion_weights = F.softmax(expansion_weights, dim=1)  # Normalize over compressed dimension
            
            # Weighted sum to expand
            expanded = torch.bmm(expansion_weights.transpose(1, 2), compressed_output)  # [batch, target_seq_len, hidden]
            return expanded
            ...
            # Create interpolation indices  
            indices = torch.linspace(0, compressed_len - 1, target_seq_len, device=device)
            floor_indices = indices.floor().long()
            ...
            raise ValueError(f"Unknown compression method for expansion: {self.compression_method}")
    ...
    def forward(self,
                ...
                current_depth: int = 0) -> Tuple[Tensor, Optional[Union[Tensor, DynamicCache]], Union[int, float]]:
        """Forward pass of soft sparse attention."""
        batch_size, seq_len, hidden_size = inputs.shape
        
        if past_key_values is not None:
            raise NotImplementedError("KV caching not yet supported for SyntaxesAttention")
        ...
        k_compressed = k_compressed.view(batch_size, k, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        if self.causal and selected_indices is not None:
            ...
            causal_mask = pos_i <= pos_j  # True where we should mask (can't attend to future)
            ...
            causal_mask = torch.triu(torch.ones(k, k, device=inputs.device), diagonal=1).bool()
        ...
        # Reshape back
        attention_output = attention_output.transpose(1, 2).contiguous().view(batch_size, k, hidden_size)
        
        # Step 3: Expand back to full sequence length
        if self.selection_method == 'interpolation':
            output = self._expand_compressed_sequence(attention_output, seq_len)
        else:
            output = self._interpolate_to_full_sequence(attention_output, selected_indices, seq_len)
