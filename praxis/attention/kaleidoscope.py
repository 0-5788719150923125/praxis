...
THE BLEND IS BASE PLUS DEVIATION, which is SMEAR's own form. ``beta_d`` is a
per-depth learned blend over the mirrors - the static preference, "mirror 2 is
generally useful at pass 3" - and ``W_turn x`` is the input-conditional
deviation on top of it. Without the static term the only way to express a
constant preference is through the input projection, which both wastes it and
CONTAMINATES ``kaleido_turn_dependence``: a learned constant smuggled through
``W_turn`` reads as input dependence that is not there. Separating them makes
the metric measure what it claims. Per-depth rather than a plain bias because it
subsumes one (depth 1 degenerates to a bias) and matches both Arc's per-depth
bias table and the facets, which are already per-depth.

Zero-init, so the blend opens uniform and every departure is learned.
``kaleido_turn_static_share`` reports which half is doing the work.
...
WHAT TO WATCH FIRST, before perplexity: ``kaleido_turn_dependence``. If it
decays to zero the router has become a constant, the mixture is a fixed matrix,
and this has silently degraded into Synthesizer's Fixed Random - the known-worse
variant.

Do NOT read that risk as inherited from SMEAR's constant-router fixed point.
That fixed point is specific to the BATCH reduction, where "the loss reaches the
routing only through ``probs.mean(0)``, so every example receives the identical
routing gradient" - measured decaying to exactly 0 on abstractinator-m. Per-token
routing has a distinct gradient per position and does not have that fixed point.
Ordinary router collapse (imbalance, one mirror winning) is still possible, which
is what ``kaleido_turn_entropy`` is paired with it for.
"""
...
NUM_MIRRORS: int = 4
# Longest sequence the frozen dictionary covers. The mirrors are indexed by
# ABSOLUTE (query, key) position - that is what lets them express patterns a
# lag-indexed kernel cannot, like "attend to the start of the sequence" - and
# the price of absolute indexing is that the span is fixed at construction.
# Cost is N * T^2 floats: 4 MB at 512, 16 MB at 1024, 268 MB at 4096. A profile
# that wants a longer span sets `span` on the class; a sequence past it raises
# rather than silently truncating.
MIRROR_SPAN: int = 1024
...
class KaleidoscopeAttention(nn.Module):
    ...
    # `turn.weight` per example, `kaleido_turn_dependence` picks up variation
    ...
    span: int = MIRROR_SPAN

    metric_descriptions = {
        "kaleido_turn_dependence": {
            "description": (
                "Does the router read its input? Normalized mutual information "
                "I(input; mirror) between the turn weights and the sequence "
                "they were computed from, in [0, 1]. THIS IS THE FIRST NUMBER "
                "TO READ, ahead of loss. Zero means every sequence received "
                "the same blend, so the mixture is a constant matrix and this "
                "block has degraded into Synthesizer's Fixed Random - the "
                "variant measured to be worse than a trained one. A constant "
                "router is a known fixed point of softmax merging in this "
                "codebase for the BATCH reduction (smear.py), though not for "
                "the per-token one this uses - so treat it as a risk to watch, "
                "not an inherited fixed point."
            ),
            "chart": {
                ...
                "series_label": "input dependence",
            },
        },
        "kaleido_turn_entropy": {
            "description": (
                "Entropy of the turn weights, normalized so 1 = a uniform "
                "blend of every mirror and 0 = one mirror taken alone. Starts "
                "at exactly 1 (the router is zero-initialised, so the opening "
                "geometry is the mean of the dictionary) and falls as the "
                "model commits. Read WITH the dependence line: falling entropy "
                "at zero dependence is collapse onto one fixed mirror, while "
                "falling entropy at rising dependence is genuine per-token "
                "selection."
            ),
            "chart": {
                "title": "Kaleidoscope Turn",
        ...
        "kaleido_turn_static_share": {
            "description": (
                ...
                "kaleido_turn_dependence: a high static share is the honest "
        ...
        "kaleido_mirror_utilization": {
            "description": (
                "Fraction of mirrors carrying more than half their fair share "
                "of the average blend - the same estimator as "
                "smear_expert_utilization. 1.0 = every mirror in the dictionary "
                "is being used; 1/N = the turn has collapsed onto one and the "
                "rest are dead weight. This is the direct read on whether N is "
                "earning its keep, and the one that says whether an N-sweep "
                "should go up or down: sitting at 1.0 with committed blend "
                "entropy argues for MORE mirrors, while a falling utilization "
                "says the dictionary is already larger than the model can use."
            ),
            "chart": {
    ...
    def __init__(
        ...
        requested = int(span or type(self).span)
        self.span = min(requested, int(getattr(config, "max_position_embeddings", 0)) or requested)
        ...
        mirrors = MIRROR_SCALE * torch.randn(
            N, self.span, self.span, generator=gen, dtype=torch.float32
        )
        self.register_buffer("mirrors", mirrors, persistent=False)

        # The turn, as base plus deviation. ``turn_static`` is the per-depth
        # learned blend (the base); ``turn`` is the input-conditional deviation.
        # Both zero-init, so the opening blend is uniform and the field is the
        # dictionary mean - an identity start in this lineage's sense, with
        # every departure from it learned. No bias on the Linear: the static
        # term already IS the bias, per depth, and having both would be a
        ...
        # The facets: one rank-1 deformation per (depth, mirror).
        self.facet_u = nn.Parameter(torch.zeros(self.depths, N, self.span))
        self.facet_v = nn.Parameter(
            torch.randn(self.depths, N, self.span) * FACET_V_STD
        )
    ...
    def _faceted(self, depth: int, T: int) -> Tensor:
        """The dictionary as this depth sees it: ``[N, T, T]``.

        Sliced to the live sequence length before the deformation is built, so
        a short sequence never materializes the full span.
        """
        d = min(int(depth), self.depths - 1)
        base = self.mirrors[:, :T, :T]
        u = self.facet_u[d, :, :T].unsqueeze(-1)  # [N, T, 1]
        v = self.facet_v[d, :, :T].unsqueeze(-2)  # [N, 1, T]
        return base + FACET_SCALE * torch.tanh(u * v)
    ...
    def forward(
        ...
        if T > self.span:
            raise ValueError(
                f"KaleidoscopeAttention was built with span={self.span} but got a "
                f"sequence of {T}. The mirrors are indexed by absolute position, so "
                f"the span is fixed at construction - raise it with a registry "
                f"profile (`span=`), at a cost of N*span^2 floats."
            )
        N, H = self.num_mirrors, self.num_heads

        d = min(int(current_depth), self.depths - 1)
        cond = self.turn(inputs).view(B, T, H, N)
        static = self.turn_static.weight[d].view(1, 1, H, N)
        w = torch.softmax((cond + static).float(), dim=-1)
        if self.training:
    ...
    def _note_turn(self, w: Tensor, cond: Tensor, static: Tensor) -> None:
        """Stash the turn diagnostics from the live blend, ``[B, T, H, N]``.

        Same estimator SMEAR uses for dependence: the gap between the entropy of
        the batch-mean blend and the mean per-sequence entropy is the mutual
        information between input and mirror choice, normalized by ``log N`` so
        it reads in [0, 1] whatever the dictionary size.

        The static share compares the two additive terms on the axis that
        actually drives the softmax - variance ACROSS mirrors, since a constant
        added to every mirror's logit cancels in the softmax and is not a
        preference at all.
        """
        n = w.shape[-1]
        if n < 2:
            return
        v_static = static.float().var(dim=-1).mean()
        v_cond = cond.float().var(dim=-1).mean()
        total = v_static + v_cond
        if float(total) > 1e-12:
            self._metrics["kaleido_turn_static_share"] = float((v_static / total).item())
        flat = w.detach().float().reshape(w.shape[0], -1, n)  # [B, T*H, N]
        h_mean = self._entropy(flat.mean(dim=0))  # [T*H]
        h_seq = self._entropy(flat).mean(dim=0)  # [T*H]
        mi = ((h_mean - h_seq) / math.log(n)).clamp(0.0, 1.0)
        self._metrics["kaleido_turn_dependence"] = float(mi.mean().item())

        # Are all N mirrors earning their keep? Same estimator as
        # smear_expert_utilization: the fraction of mirrors carrying more than
        # half their fair share of the average blend. 1.0 = every mirror is
        # used, 1/N = the dictionary has collapsed onto one and the rest are
        # dead weight, which is the direct read on whether N is too large.
        usage = flat.mean(dim=(0, 1))  # [N] - mean blend weight per mirror
        self._metrics["kaleido_mirror_utilization"] = float(
            (usage > (0.5 / n)).float().sum().item() / n
        )
        self._metrics["kaleido_turn_entropy"] = float(
            (h_seq.mean() / math.log(n)).clamp(0.0, 1.0).item()
        )
