...
class PraxisCache(DynamicCache):
    def __init__(self) -> None:
        ...
        # Classifier-side decode state, keyed by the owning module (a harmonic
        # field's running prefix mean and fast-weight bank, for instance). Kept
        # apart from ``states`` on purpose: ``past_length()`` reads the trunk's
        # own slots and must not see the classifier's bookkeeping, since the
        # classifier derives its position offset FROM past_length().
        self.classifier_states: Dict[str, Dict[str, Any]] = {}
    ...
    def get_classifier_state(self, key: str) -> Optional[Dict[str, Any]]:
        return self.classifier_states.get(key)

    def set_classifier_state(self, key: str, state: Dict[str, Any]) -> None:
