...
class BaseRegularizer(nn.Module):
    ...
    regularizer wants: ``scorer`` (the output readout), ``classifier`` (the
    whole classifier, for a term that targets something inside it), and
    ``main_loss`` (this step's main term, for a term that balances itself
    against what the task will pay). Accept and ignore what you don't use - the
