...
def _series(metrics, prefix, max_loops):
    ...
    counts = [float(metrics.get(f"halting/{prefix}_r_{r}", 0.0)) for r in range(1, max_loops + 1)]
...
def render_png(name, metrics):
    ...
    for i, (counts, lbl, col) in enumerate(present):
        ...
        ax.bar(x + (i - (len(present) - 1) / 2) * width, frac, width, label=lbl, color=col)
...
def export_halting() -> dict:
    ...
    if not found:
        with open(OUT_TEX, "w") as fh:
            fh.write("% Generated by praxis/pillars/halting.py - no halting data found.\n"
                     "\\newcommand{\\paperHaltingFigure}{}\n")
        return {"run": None}
