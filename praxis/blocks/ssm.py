...
class SSMBlock(nn.Module):
    ...
    def __init__(self, config: ConfigType, *args: Any, **kwargs: Any) -> None:
        ...
        self.in_proj = nn.Linear(self.hidden_size, self.hidden_size * self.expand_factor * 2)
        ...
        # SSM parameters
        self.x_proj = nn.Linear(self.hidden_size * self.expand_factor, self.dt_rank + self.state_size * 2)
        ...
        A = torch.arange(1, self.state_size + 1).float().repeat(self.hidden_size * self.expand_factor, 1)
        ...
        # Output projection
        self.out_proj = nn.Linear(self.hidden_size * self.expand_factor, self.hidden_size)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        delta, B, C = x_proj_out.split([self.dt_rank, self.state_size, self.state_size], dim=-1)
        ...
        if current_state is None:
            current_state = torch.zeros(
                batch_size, self.hidden_size * self.expand_factor, self.state_size,
        ...
        cumsum_deltaA_shifted = torch.cat([
            torch.ones_like(cumsum_deltaA[:, :1]),
            cumsum_deltaA[:, :-1]
        ], dim=1)
