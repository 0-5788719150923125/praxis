...
class PraxisTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        residual, beta = (
            self.attn_res.connect_width(inputs) if self.attn_res else (inputs, None)
        )
        normalized = self.attn_norm(inputs)
        outputs = self.attn(normalized, attention_mask)
        outputs = (
            self.attn_res.connect_depth(residual, outputs, beta)
            if self.attn_res
            else outputs + residual
        )
        residual, beta = (
            self.ffn_res.connect_width(outputs) if self.ffn_res else (outputs, None)
        )
        ...
        outputs = (
            self.ffn_res.connect_depth(residual, outputs, beta)
            if self.ffn_res
            else outputs + residual
        )
