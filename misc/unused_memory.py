...
# source: https://github.com/dingo-actual/infini-transformer/blob/main/infini_transformer/compressive_memory.py
class CompressiveMemory(nn.Module):
    ...
    def forward(
        ...
        k_full = (
            self.proj_k(x)
            .unsqueeze(1)
            .view((batch_size, self.num_heads, x.size(1), self.dim_key))
        )
        v_full = (
            self.proj_v(x)
            .unsqueeze(1)
            .view((batch_size, self.num_heads, x.size(1), self.dim_value))
        )
        q_full = (
            self.proj_q(x)
            .unsqueeze(1)
        ...
        for ix in range(num_segments):
            ...
            if sample_mask is not None:
                if self.sampling_factor is None:
                    raise ValueError(
                        "sampling_factor must be specified if sample_mask is provided"
                    )
